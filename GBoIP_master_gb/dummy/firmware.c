...
DDRD = 0b00010000;		//PD2 - clock signal on int0, PD3 - data from local gameboy, PD3 - data to local gameboy

PORTD&=~(1<<PORTD4);    	// pull data line to local gameboy low
//Netzwerk
...
if(ser_out& _BV(7)){			//write bit to local gameboy
	PORTD|=(1<<PORTD4);             // 1
} else {
	PORTD&=~(1<<PORTD4);    	// 0
}

if(PIND& _BV(PD3)){			// read bit from local gameboy
	ser_in |= _BV(0);
}
