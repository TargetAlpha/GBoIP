...
volatile uint8_t ser_in_p1 = 0;
volatile uint8_t ser_in_p2 = 0;
volatile uint8_t ser_in_p1_buffer = 0;
volatile uint8_t ser_in_p2_buffer = 0;
volatile uint8_t ser_out_p1 = 0;
volatile uint8_t ser_out_p2 = 0;
	...
	while (1) { // Main loop start.
		...
		if (received == 0) {
			if (send == 1){
				...
				send_udp(buffer, wort, 1, 37351, serverip, 33750, gwmac);				
				send = 0;
			}
			continue;
		}
